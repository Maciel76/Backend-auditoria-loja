  ...
  async (req, res) => {
    try {
      ...
      const [
        ...
        MetricasUsuario.find({ loja: req.loja._id })
          .sort({ ultimaAtualizacao: -1 })
          .limit(5),
        MetricasLoja.find({ loja: req.loja._id })
          .sort({ ultimaAtualizacao: -1 })
          .limit(3),
      ]);
      ...
      if (totalAuditorias > 0 && totalMetricasUsuario === 0) {
        status.recomendacoes.push(
          "🔄 Execute o cálculo de métricas manualmente",
        );
      }
...
router.post("/calcular-agora", verificarLojaObrigatoria, async (req, res) => {
  try {
    ...
    console.log(
      `🔄 Forçando cálculo de métricas ${periodo} para loja ${req.loja.codigo}...`,
    );
  ...
  try {
    ...
    if (confirmar !== "SIM_LIMPAR_TUDO") {
      return res.status(400).json({
        ...
        instrucoes:
          "Envie { 'confirmar': 'SIM_LIMPAR_TUDO' } para limpar todas as métricas",
      });
    }
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const usuariosUnicos = await Auditoria.distinct("usuarioId", {
        loja: req.loja._id,
      });
      ...
      const topUsuarios = await MetricasUsuario.find({ loja: req.loja._id })
        ...
        .limit(10)
        .select(
          "usuarioId usuarioNome totais.pontuacaoTotal periodo dataInicio",
        );
      ...
      res.json({
        ...
        auditorias_raw: {
          ...
          total_geral: statsAuditorias.reduce(
...
router.get("/testar-servico", async (req, res) => {
  try {
    ...
    try {
      periodoTest = metricsCalculationService.obterPeriodo(
        "mensal",
        new Date(),
      );
    } catch (err) {
...
router.get("/testar-conexoes", async (req, res) => {
  try {
    ...
    const resultados = {
      auditoria:
        testes[0].status === "fulfilled" ? "✅ OK" : `❌ ${testes[0].reason}`,
      metricas_usuario:
        testes[1].status === "fulfilled" ? "✅ OK" : `❌ ${testes[1].reason}`,
      metricas_loja:
        testes[2].status === "fulfilled" ? "✅ OK" : `❌ ${testes[2].reason}`,
      metricas_auditoria:
        testes[3].status === "fulfilled" ? "✅ OK" : `❌ ${testes[3].reason}`,
      metricas_globais:
        testes[4].status === "fulfilled" ? "✅ OK" : `❌ ${testes[4].reason}`,
      loja:
        testes[5].status === "fulfilled" ? "✅ OK" : `❌ ${testes[5].reason}`,
      user:
        testes[6].status === "fulfilled" ? "✅ OK" : `❌ ${testes[6].reason}`,
    };

    const todosOk = Object.values(resultados).every((r) => r.includes("✅"));

    res.json({
      status_geral: todosOk
        ? "✅ Todas as conexões OK"
        : "⚠️ Algumas conexões com problema",
      conexoes: resultados,
      timestamp: new Date(),
    });
  } catch (error) {
  ...
  async (req, res) => {
    try {
      ...
      const metricasUsuarios = await MetricasUsuario.find({
        ...
        .limit(5)
        .select(
          "usuarioId usuarioNome contadoresAuditorias totaisAcumulados historicoRanking totais",
        );
      ...
      const resultado = {
        ...
        exemplos_usuarios: metricasUsuarios.map((m) => ({
          ...
          ranking_historico: {
            melhor_posicao:
              m.historicoRanking?.melhorPosicao || "Nunca no top 10",
        ...
        estatisticas_gerais: {
          ...
          usuarios_com_contadores: metricasUsuarios.filter(
            (m) => m.contadoresAuditorias?.totalGeral > 0,
          ).length,
          usuarios_com_historico: metricasUsuarios.filter(
            (m) => m.totaisAcumulados?.itensLidosTotal > 0,
          ).length,
          usuarios_ja_no_ranking: metricasUsuarios.filter(
            (m) => m.historicoRanking?.totalTop10 > 0,
          ).length,
        },
